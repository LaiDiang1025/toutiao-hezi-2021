...
export default {
  ...
  data () {
    return {
        ...
        extensions: [
            ...
            new Strike(),   //删除线
            new Image({
                //默认会把图片生成base64 和内容存在一起
                // 自定义图片上传
                uploadRequest(file){    // 图片的上传方法，返回一个 Promise<url>
                    const fd = new FormData()
                    fd.append('image',file)
                    // 第一个return 是返回 Promise对象？ 因为axios本身就是返回Promise对象
        ...
        formRules:{
            ...
            content:[
                // { validator,message: '请输入文本内容', trigger: 'blur'}
                {
  ...
  created () {
      ...
      //  由于让发布和修改使用的同一个组件
      // 需要判断url，如果路径请求参数中有id，则请求展示文字内容
